...
export default function RegularNeeds() {
  ...
  async function fetchRegularNeeds() {
    setLoading(true);

    const { data, error } = await supabase
      .from("products")
      .select("*")
      .eq("tag", "regular-needs")
  ...
  // Group into pairs (2 items per slide)
  const slides = [];
  ...
  return (
    <View className="mt-14 px-3">
      ...
      <View className="bg-[#C9D9FF] rounded-3xl p-4 relative">
        ...
        {/* Loader */}
        {loading && (
          <ActivityIndicator size="large" color="#000" className="py-10" />
        )}
              ...
              renderItem={({ item }) => (
                <View style={{ width: SLIDE_WIDTH }} className="flex-row justify-around px-2">
                  ...
                  {item.map((prod: Product) => (
                    <TouchableOpacity
                      key={prod.id}
                      onPress={() => openProduct(prod.id)} // open by product ID
